...
func DecideLogin(snapshot CurrentSessionSnapshot, command LoginCommand) (Mutation, error) {
	duplicate, err := prepareTransition(snapshot, command.Key, command.Timestamp, command.EventID)
	if err != nil {
		return nil, err
	}
	if duplicate {
		return DuplicateEvent{EventID: command.EventID, Timestamp: command.Timestamp}, nil
	}
...
func prepareTransition(snapshot CurrentSessionSnapshot, key SessionKey, timestamp time.Time, eventID string) (bool, error) {
